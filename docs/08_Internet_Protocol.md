...
An IPv4 address is assigned to the network interface connection for a host. This connection is usually a network interface card (NIC) installed in the device. Some servers can have more than one NIC and each of these has its own IPv4 address. Router interfaces that provide connections to an IP network will also have an IPv4 address.
...
IPv4 addresses are 32 bits in length. Example of an IPv4 address in binary: **11010001101001011100100000000001**.

Notice how difficult this address is to read. For this reason, the 32 bits are grouped into four 8-bit bytes called octets like this: **11010001.10100101.11001000.00000001**.

It's better but still difficult to read. That is why each octet is converted into its decimal value, separated by a decimal point or period. The above binary IPv4 becomes the below dotted-decimal representation: **209.165.200.1**.

## IPv4 Structure
The logical 32-bit IPv4 address is hierarchical and is made up of two parts, the network, and the host.

- For example, there is a host with an IPv4 address 192.168.5.11 with a subnet mask of 255.255.255.0. The first three octets (192.168.5), identify the network portion of the address, and the last octet (11) identifies the host.
